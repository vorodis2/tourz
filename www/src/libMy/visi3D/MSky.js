...
export default function MSky (_parent) {
	...
	this.dergLink = function () {
		console.warn(">>dergLink::@@@@@@@@@",this._link); 
		...
		if (this._link == 'null' || this._link == null) { // сносим мапу
			...
			if (this.material.map != null) {
				...
				console.warn(">>dLloader1::",self._link);				
			}
			...
			/*console.warn(">>dLloader1::",self._link);
			this.loader.load(, function (textur) {
				...
				self.material.needsUpdate = true;
				self.parent.parent.intRend = 1;
				//trace(">>>>>>>>333333>>>>>>>>",this._link,"fgh",self.onComplete)
				console.warn(">>dLloader1::",self.textur.linkMy); 
				if(self.onComplete)self.onComplete()
			});*/
			...
			if (tt == 0) { // щбычная
				...
				this.loader.load(this._link, function (textur) {
					...
					self.material.needsUpdate = true;
					self.parent.parent.intRend = 1;
					console.warn(">>dLloader2::",self.textur.linkMy); 
					if(self.onComplete)self.onComplete()
				},function(e){
					trace(e)
				},function(e){
					trace("An error happened  ",e)
				});
			}
			if (tt == 1) { // hdr
